...
### 变更
- 物品图标改为 **两层结构**：底层按 id 引用原版铁轨贴图（`minecraft:block/rail`，jar 内不打包原版素材），上层是自绘的**彩色圆球标记**——左开画在左半（蓝球）、右开画在右半（橙球），位置本身即说明往哪边岔。
...
- 新增 `tools/check_compliance.py`：断言 jar 内不含 `assets/minecraft/**`，且自带贴图与原版贴图的非透明像素重合率为 0。
...
- README 新增「合规」章节，写清素材来源（世界内铁轨按 id 引用原版贴图，jar 内不打包任何原版素材）。
